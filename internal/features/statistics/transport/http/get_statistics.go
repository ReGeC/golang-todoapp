...
// @Description  Получение статистики по задачам в системе с опциональной фильтрацией по user_id и/или временному промежутку
// @Tags 		 statistics
// @Produce 	 json
// @Param 		 user_id  query    int     false  "фильрация задач для статистики по id автора"
...
func getUserIDFromToQueryParams(r *http.Request) (*int, *time.Time, *time.Time, error) {
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("qet `userID` query param: %w", err)
	}

	from, err := core_http_request.GetDateQueryParam(r, fromQueryParamKey)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("qet `from` query param: %w", err)
	}

	to, err := core_http_request.GetDateQueryParam(r, toQueryParamKey)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("qet `to` query param: %w", err)
	}
