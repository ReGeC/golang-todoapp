...
// @Description Получение задач в системе с опциональной пагинацией
// @Tags 		tasks
// @Produce 	json
// @Param 		user_id	query int false "фильрация задач по id автора"
...
func (h *TasksHTTPHandler) GetTasks(rw http.ResponseWriter, r *http.Request) {
	...
	response := GetTasksResponse(arrayTaskDTOFromDomains(taskDomains))
...
func getUserIDLimitOffsetQueryParams(r *http.Request) (*int, *int, *int, error) {
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("qet `limit` query param: %w", err)
	}

	offset, err := core_http_request.GetIntQueryParam(r, offsetQueryParamKey)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("qet `offset` query param: %w", err)
	}
